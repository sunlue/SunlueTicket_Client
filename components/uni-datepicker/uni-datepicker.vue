	...
	export default {
		...
		methods: {
			...
			procCalendar(item) {
				...
				if(item.dateObj.getTime()<new Date(this.minDate).getTime()){
					item.statusStyle.color = 'rgb(221, 221, 221)';
				}
				if(item.dateObj.getTime()>new Date(this.maxDate).getTime()){
					item.statusStyle.color = 'rgb(221, 221, 221)';
				}
			},
			//刷新日历
			...
			//选中日期
			onSelectDate(date) {
				if(date.dateObj.getTime()<new Date(this.minDate).getTime()){
					return false;
				}
				if(date.dateObj.getTime()>new Date(this.maxDate).getTime()){
					return false;
				}
